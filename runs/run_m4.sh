...
LANGS='nso,msa,tgl,cat,eng'
LANG_PAIRS='nso-eng,msa-eng,tgl-eng,cat-eng,eng-nso,eng-msa,eng-tgl,eng-cat'
SRCS='nso,msa,tgl,cat'
tgt=eng
...
## Train 
 ...
 python train.py --distributed-world-size 32 --distributed-port ${RANDOM_PORT} ${DATA_BIN} --arch ${ARCH}  --task translation_multi_simple_epoch \
 --moe-expert-count ${EXPERT_NUM} --moe-freq ${MOE_FREQ} ${EOM} ${CMR}  --hmoe-layer ${HMOE_LAYER} --only-compute-current-layer-tokens 1 --hmoe-layernorm 1 --hmoe-dynamic ${DYNAMIC_LAYER} \
