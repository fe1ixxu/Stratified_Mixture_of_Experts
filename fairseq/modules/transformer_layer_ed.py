...
from fairseq.modules.moe import CMRLayer, MOELayer, Top1Gate, Top1GateMask, Top2Gate, Top2GateMask, HMOELayer
...
class TransformerEncoderLayerBaseED(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False, is_moe_layer=False):
        ...
        self.hmoe_layer = cfg.hmoe_layer if build_moe and cfg.hmoe_layer else None
        self.only_compute_current_layer_tokens = cfg.only_compute_current_layer_tokens if build_moe and cfg.only_compute_current_layer_tokens else None
        self.hmoe_layernorm = cfg.hmoe_layernorm if build_moe and cfg.hmoe_layernorm else None
        if build_moe:
            if self.hmoe_layer:
                self.total_expert_num = cfg.moe_expert_count
                self.expert_num_per_layer = cfg.hmoe_dynamic
                if self.expert_num_per_layer:
                    self.expert_num_per_layer = [int(l) for l in self.expert_num_per_layer.split(",")]
                    assert sum(self.expert_num_per_layer) == self.total_expert_num and len(self.expert_num_per_layer) == self.hmoe_layer
                else:
                    assert cfg.moe_expert_count % self.hmoe_layer == 0
                    self.expert_num_per_layer = [int(cfg.moe_expert_count / self.hmoe_layer)] * self.hmoe_layer
                self.layer_ranges = [[sum(self.expert_num_per_layer[:i]), sum(self.expert_num_per_layer[:i+1])] for i, l in enumerate(self.expert_num_per_layer)]
                lang_idx = None
                if cfg.cmr_log_lang_gates:
                    lang_idx = getattr(cfg, "lang_idx", None)
                    assert lang_idx is not None, cfg
                if cfg.moe_top1_expert:
                    gate = nn.ModuleList([Top1GateMask(
                        self.embed_dim,
                        self.total_expert_num - sum(self.expert_num_per_layer[:i]),
                        ...
                        mask_expert=sum(self.expert_num_per_layer[:i]),
                    ) for i in range(self.hmoe_layer)])
                else:
                    gate = nn.ModuleList([Top2GateMask(
                        self.embed_dim,
                        self.total_expert_num - sum(self.expert_num_per_layer[:i]),
                ...
                if self.hmoe_layernorm:
                    self.hmoe_layernorms = nn.ModuleList([LayerNorm(self.embed_dim) for _ in range(self.hmoe_layer)])
                else:
                    self.hmoe_layernorms = None

                # TODO: HMOE for moe_cmr
                # if cfg.moe_cmr:
    ...
    def forward(
        ...
        if not run_moe:
            ...
            l_aux = None
        elif run_moe and (not self.hmoe_layer):
            x, fc_result, l_aux = self.forward_moe(
                x,
                tokens,
            )
        else:
            x, fc_result, l_aux = self.forward_hmoe(
                x,
                tokens,
                residual,
            )
        if not self.hmoe_layer:
            x = self.residual_connection(x, residual)
        if not self.normalize_before:
            if not self.hmoe_layernorm:
                x = self.final_layer_norm(x)
    ...
    def forward_hmoe(self, x, tokens, residual):
        x = x.transpose(0, 1)  # batch_size, seq_len, model_dim
        residual = residual.transpose(0, 1)
        input_shape = x.shape

        # TODO: for cmr loss?
        l_aux_total = {"moe_gate_loss": 0}
        ## initialize current layer states, [bz, seq_len]
        current_layer_state = torch.zeros(input_shape[0], input_shape[1]).to(x.device)
        current_lowest_layer = 0
        pass_count = 0
        while current_lowest_layer < self.hmoe_layer:
            mask_lowest_layer = torch.where(current_layer_state == current_lowest_layer, 1, 0)
            
            x2, fc_result, l_aux = self.forward_hmoe_sublayer(
                x,
                tokens,
                int(current_lowest_layer),
                mask_lowest_layer,
                self.only_compute_current_layer_tokens,
            )
            x = x2 * mask_lowest_layer.unsqueeze(-1) + x * (-mask_lowest_layer + 1).unsqueeze(-1)

            l_aux_total["moe_gate_loss"] += l_aux["moe_gate_loss"]

            gate_logits = l_aux["soft_gates"].reshape(input_shape[0], input_shape[1], -1)
            _, gate_selections = gate_logits.max(dim=-1)
            
            new_layer_state = current_layer_state
            for ind, layer_range in enumerate(self.layer_ranges[int(current_lowest_layer):]):
                ind += current_lowest_layer + 1
                new_layer_state = torch.where(
                    (gate_selections >= layer_range[0]) & (gate_selections < layer_range[1]),
                    ind,
                    new_layer_state
                    )
            new_layer_state = new_layer_state * mask_lowest_layer + current_layer_state * (-mask_lowest_layer + 1)
            current_layer_state = new_layer_state
            current_lowest_layer = torch.min(current_layer_state)
            local_current_lowest_layer = current_lowest_layer.clone()
            current_lowest_layer = int(
            ...
            # layers may differ in different gpus, e.g., lowest_layer in gpu0 is 2 while 3 in gpu3
            x = x + residual * mask_lowest_layer.unsqueeze(-1)
            if local_current_lowest_layer == current_lowest_layer:
                residual = x
                if self.hmoe_layernorms:
                    x[torch.where(current_layer_state == current_lowest_layer)] = self.hmoe_layernorms[int(current_lowest_layer)-1](x[torch.where(current_layer_state == current_lowest_layer)])
            pass_count += 1

        l_aux_total["moe_gate_loss"] = l_aux_total["moe_gate_loss"] / pass_count
        
        x = x.transpose(0, 1)  # seq_len, batch_size, model_dim
        return x, fc_result, l_aux_total

    def forward_hmoe_sublayer(self, x, tokens, layer_id, mask_lowest_layer, only_compute_current_layer_tokens):
        fc_result = None
        ...
        if self.cfg.use_moe_pad_mask:
            x, l_aux = moe_module(
                ...
                prefix_tokens=prefix_tokens,
                layer_id=layer_id,
                mask_lowest_layer=mask_lowest_layer,
                only_compute_current_layer_tokens=only_compute_current_layer_tokens,
            )
        else:
            x, l_aux = moe_module(
                x, 
    ...
    def forward_moe_ed(self, x, tokens, fc3, activation_fn):
        ...
        # Move forward to fc3
        if has_fused_bias_gelu and activation_fn == gelu:
            x1 = fused_bias_gelu(x1, fc3.bias)
            x1 = _linear(x1, fc3.weight)
            x2 = fused_bias_gelu(x2, fc3.bias)
            x2 = _linear(x2, fc3.weight)
        elif activation_fn == relu_squared:
            x1 = fused_bias_relu_squared(x1, fc3.bias)
            x1 = _linear(x1, fc3.weight)
            x2 = fused_bias_relu_squared(x2, fc3.bias)
            x2 = _linear(x2, fc3.weight)
        else:
            x1 = activation_fn(x1)
...
class TransformerEncoderLayerED(TransformerEncoderLayerBaseED):
...
class TransformerDecoderLayerBaseED(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        if not self.is_moe_layer or self.cfg.alternate_decoder_ffn_embed_dim > 0:
            ...
            l_aux = None
        elif self.is_moe_layer and (not self.hmoe_layer):
            x, l_aux = self.forward_moe(x, tokens)
        else:
            x, l_aux = self.forward_hmoe(x, tokens, residual)
    ...
    def forward_hmoe(self, x, tokens, residual):
        ...
        while current_lowest_layer < self.hmoe_layer:
            mask_lowest_layer = torch.where(current_layer_state == current_lowest_layer, 1, 0)
            x2, l_aux = self.forward_hmoe_sublayer(
                x,
                tokens,
                int(current_lowest_layer),
                mask_lowest_layer,
        ...
        x = x.transpose(0, 1)  # seq_len, batch_size, model_dim
        l_aux_total["moe_gate_loss"] = l_aux_total["moe_gate_loss"] / pass_count
        return x, l_aux_total

    def forward_moe_ed(self, x, tokens, fc3, activation_fn):
        x1, l_aux = self.forward_moe(
            x,
            tokens,
        )
        x2 = x.detach()
        x2, _, = self.forward_moe(
            x,
            tokens,
        )
...
# backward compatible with the legacy argparse format
class TransformerDecoderLayerED(TransformerDecoderLayerBaseED):
    def __init__(
        self,
        args,
        no_encoder_attn=False,
