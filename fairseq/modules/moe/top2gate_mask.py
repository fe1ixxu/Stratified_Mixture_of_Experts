...
def top2gatingmask(
    ...
    if use_fp32:
        ...
        logits = logits.float()
    if mask_expert:
        num_true_experts = logits.shape[1]
        pad_logits = torch.ones(logits.shape[0], mask_expert, device=logits.device, dtype=logits.dtype) * float("-inf")
    ...
    if moe_eval_capacity_token_fraction > 0.0 and eval_mode:
        ...
        # capacity = 2S/E
        if mask_expert:
            capacity = 2 * math.ceil(num_tokens / num_true_experts)
        else:
    ...
    if prefix_tokens is not None:
        assert prefix_tokens is None, "prefix_tokens not supported for hmoe now!"
    ...
    if batch_prioritized_routing:
        assert not batch_prioritized_routing, "batch_prioritized_routing not support now!"
        # if batch_prioritized_routing:
    ...
    # Compute l_aux
    me = torch.mean(gates[:, mask_expert:], dim=0)
    ce = torch.mean(mask1[:, mask_expert:].to(gates.dtype), dim=0)

    l_aux = torch.mean(me * ce)

    if mask_expert:
        l_aux = l_aux * num_true_experts * num_true_experts
    else:
    ...
    expert1_hist = (
        100
        * torch.histc(
            (indices1_s.squeeze() + 1), bins=num_experts, min=mask_expert+1, max=num_experts
        )
        / num_tokens
    )
    ...
    expert2_hist = (
        100
        * torch.histc(
            (indices2_s.squeeze() + 1), bins=num_experts, min=mask_expert+1, max=num_experts
        )
        / num_tokens
    )
    ...
    if mask_expert:
        sample_count = max(math.ceil(num_true_experts * SAMPLE_FRACTION), 1)
    else:
    ...
    if use_tutel:
        assert not use_tutel, "use_tutel not supported now!"
...
class Top2GateMask(torch.nn.Module):
