...
class HMOELayer(Base):
    ...
    def forward(
        self, *input: Tensor, layer_id, input_padding_mask=None, prefix_tokens=None, mask_lowest_layer=None, only_compute_current_layer_tokens=None, **kwargs: Any
    ) -> Tensor:
        ...
        num_tokens = None
        mask_lowest_layer = mask_lowest_layer.reshape(-1)
        ...
        if expected_bsz == 0:
            if only_compute_current_layer_tokens:
                expected_dim = int(
                    distributed_utils.all_reduce(
                        torch.sum(mask_lowest_layer)
                ...
                num_tokens = int(torch.sum(mask_lowest_layer))
                padded_input[: num_tokens, :] = reshaped_input[torch.where(mask_lowest_layer == 1)]
                ...
                if reshaped_input_padding_mask is not None:
                    padded_input_padding_mask[
                        : num_tokens
                    ] = reshaped_input_padding_mask[torch.where(mask_lowest_layer == 1)]
                else:
                    padded_input_padding_mask[: num_tokens] = False
                ...
                if reshaped_prefix_tokens is not None:
                    ...
                    padded_prefix_tokens[: num_tokens] = reshaped_prefix_tokens[torch.where(mask_lowest_layer == 1)]
                ...
                reshaped_input = padded_input
                reshaped_input[torch.where(mask_lowest_layer != 1)] = 0
                ...
                if reshaped_input_padding_mask is not None:
                    padded_input_padding_mask[
                        : reshaped_input_shape[0]
                    ][torch.where(mask_lowest_layer==1)] = reshaped_input_padding_mask
                else:
                    padded_input_padding_mask[: reshaped_input_shape[0]][torch.where(mask_lowest_layer==1)] = False
                reshaped_input_padding_mask = padded_input_padding_mask
                if reshaped_prefix_tokens is not None:
                    padded_prefix_tokens = torch.zeros(
                        (expected_dim,),
        ...
        if self.use_tutel:
            l_aux, self.metadata, C, E, indices_, locations_, gates_ = self.gate[layer_id](
                reshaped_input,
                reshaped_input_padding_mask,
            ...
            l_aux, combine_weights, dispatch_mask, self.metadata = self.gate[layer_id](
                reshaped_input,
                reshaped_input_padding_mask,
                eval_capacity_length,
                prefix_tokens=reshaped_prefix_tokens,
        ...
        if only_compute_current_layer_tokens:
            ## Only support --max-tokens
            soft_gates = self.metadata["soft_gates"]
            new_soft_gates = torch.zeros(
                reshaped_input_shape[0], soft_gates.shape[1],
                device=soft_gates.device,
                dtype=soft_gates.dtype,
            )
            new_soft_gates[torch.where(mask_lowest_layer==1)] = soft_gates[ : num_tokens].clone()
            soft_gates = new_soft_gates

            new_combined_output = torch.zeros(
                reshaped_input_shape,
                device=input.device,
                dtype=input.dtype,
            )
            combined_output = combined_output[: num_tokens, :]
            new_combined_output[torch.where(mask_lowest_layer==1)] = combined_output
            combined_output = new_combined_output.reshape(input.shape)
        else:
            soft_gates = self.metadata["soft_gates"][: reshaped_input_shape[0], :]
        ...
        return combined_output, {"moe_gate_loss": l_aux, "soft_gates": soft_gates}
